function MatTracking=ULM_Superloc2D_mesh(MatIn,ULM)
%% function MatTracking=ULM_Superloc2D_mesh(MatIn,ULM)
...
% Last modified on 02/10/2019 By Arthur Chavngon
...
% averageXc/Zc will be the weighted matrices with the weighted averages
% [MatOut,MatTracking]=superloc2D_SAF(MatIn,fwhmz,fwhmx,numberOfParticles);
%
...
for iscat=1:size(MatIn,3)
    IntensityRoi  = abs(MatIn(index_mask_z(iscat)+vectfwhmz,index_mask_x(iscat)+vectfwhmx,iscat));
