function val = myBoxPlotFC_multi(label_in,val_Err,labelSize,InPutColor,txtsize,dispVal)
%% function val = myBoxPlotFC_multi(label_in,val_Err,labelSize,InPutColor,txtsize,dispVal)
...
val_med = squeeze(val_med);
val_std = squeeze(val_std);
val_d1 = squeeze(val_d1);
val_d9 = squeeze(val_d9);
val_q1 = squeeze(val_q1);
val_q3 = squeeze(val_q3);
...
%         rectangle('position',posrect,'edgecolor','none','FaceColor',col*.9)  
        ...
        text(ii+box_width/2 +posX(ia)-.1,val_d9(ii,ia)+.02,...
            ...
            'fontsize',txtsize,'interpreter','tex','HorizontalAlignment','left','VerticalAlignment','middle','color','k','Rotation',80) 
        end
    end
